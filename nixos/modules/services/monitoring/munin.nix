  ...
  muninPlugins = pkgs.stdenv.mkDerivation {
    ...
    buildCommand = ''
      ...
      for file in $out/*; do
        ...
        wrapProgram $file \
          --set PATH "${config.security.wrapperDir}:/run/current-system/sw/bin:/run/current-system/sw/bin" \
  ...
  config = mkMerge [ (mkIf (nodeCfg.enable || cronCfg.enable)  {
    ...
    systemd.services.munin-node = {
      ...
      preStart = ''
        ...
        PATH="${config.security.wrapperDir}:/run/current-system/sw/bin:/run/current-system/sw/bin" ${pkgs.munin}/sbin/munin-node-configure --shell --families contrib,auto,manual --config ${nodeConf} --libdir=${muninPlugins} --servicedir=/etc/munin/plugins 2>/dev/null | ${pkgs.bash}/bin/bash
      '';
      serviceConfig = {
